...
CHANCE_MUT = .15      # Chance de mutação de um peso qualquer
CHANCE_CO = .35      # Chance de crossing over de um peso qualquer
NUM_INDIVIDUOS = 19  # Tamanho da população
NUM_MELHORES = 12     # Número de indivíduos que são mantidos de uma geração para a próxima
    ...
    for i in range(3):
        for j in range(10):
            if np.random.uniform(0, 1) < CHANCE_MUT:
                individuo[i][j] *= np.random.uniform(1.2, 1.5)
